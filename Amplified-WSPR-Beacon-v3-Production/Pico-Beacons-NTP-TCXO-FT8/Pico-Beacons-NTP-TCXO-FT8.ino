...
long long frequency = 28075000 * 100UL; // CHANGE THIS PLEASE
...
void tx(uint8_t *tones)
{
  ...
  digitalWrite(SW1, LOW); // Enable bias for MRF101 EVAL BOARD
  digitalWrite(SW2, HIGH); // PTT
  ...
  digitalWrite(SW1, HIGH); // Disable bias
  // digitalWrite(SW2, LOW); // Keep relay engaged (HIGH)!
...
void setup() {
  ...
  pinMode(SW1, OUTPUT);
  pinMode(SW2, OUTPUT);
  digitalWrite(SW1, HIGH); /* Turn off bias */
  digitalWrite(SW2, HIGH); // Keep PTT turned on!
  Serial.begin(115200);
  delay(5000);
  ...
  // Set CLK0 output
  si5351.drive_strength(SI5351_CLK0, SI5351_DRIVE_8MA); // Set for minimum power as we have an external amplifier
