...
//
// Capable of running off the RTC without internet once RTC is synced
...
long long frequency = 28126100 * 100UL; // CHANGE THIS PLEASE
long long new_frequency = 0;
uint8_t tones[255];
int toneDelay;
int symbolCount;
int toneSpacing;
...
char loc[] = "MK68"; // CHANGE THIS PLEASE!
// https://physics.princeton.edu//pulsar/K1JT/doc/wspr/wspr-main.html#CONVERSION
uint8_t dbm = 23; // 200mW
JTEncode jtencode;

// Pins
#define EXTERNAL_LED 2 // GP2
...
void setup() {
  ...
  Serial.printf("Device is configured with (%s %s %d) settings...\n", call, loc, dbm);
  Serial.printf("Device is configured with (%s %s %d) settings...\n", call, loc, dbm);
  Serial.printf("Device is configured with (%s %s %d) settings...\n", call, loc, dbm);
  ...
  if (buttonState == HIGH) {
    ...
    if (multi.run() != WL_CONNECTED) {
      Serial.println("Unable to connect to network, using time from DS3231...");
      // delay(10000);
      // rp2040.reboot();
    } else {
      Serial.println("");
...
static int sidx = 0; // serial index
  ...
  if (count > 0) {
    ...
    if (s != e && s != NULL && e != NULL ) { /* We may have a valid message */
      if (*(s + 1) == 'v') {
        ...
        char *f = strtok(s + 2, "*");
        char *ptr;
        new_frequency = strtoul(f, &ptr, 10);
      }
      // Clear stuff
      sidx = 0;
